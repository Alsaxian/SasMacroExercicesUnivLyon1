...
libname TP2 'C:\Users\ellie\Documents\My SAS Files\Lyon';
filename patients 'C:\Users\ellie\Documents\My SAS Files\Lyon';
filename pat2 'C:\Users\ellie\Documents\My SAS Files\Lyon\dosePat.csv';

/* Question 1 : importer le fichier dose_patients */
data tp2.dose_patients;
	length statut $15;
	infile patients(dose_patients.csv) firstobs=2  DLM=";"; /* Use fielname command like that!! */
	input identifiant statut$ biomarqueur dose;
run;

/* Une procédure existe pour également importer un fichier : proc import */
proc import 
	datafile=pat2 out=dose_patients_bis DBMS=CSV replace; /* But! datafile=patients(dose_patients.csv) won't work! For unknown reasons. */
	delimiter=";"; 
run;
...
ods rtf file='C:\Users\ellie\Documents\My SAS Files\Lyon\proccontents.rtf'; /* Can be opened with a notepad or Word! Good quality! */

	proc contents data=tp2.dose_patients;
	run;
...
/* Question 3 */
proc print data=tp2.dose_patients noobs;
	format dose 10.1;
run;
...
infile 'U:\TP SAS\TP2\info_patient.txt' firstobs=2  DLM='09'x;
input id_patient age sexe$ fumeur$;
run;

/* Autre option avec la proc import :*/
proc import datafile="U:\TP SAS\TP2\info_patient.txt" out=data.info_patient2 dbms=TAB replace;
run;
...
/* Question 7 : Transposition de la table de données */
...
/* Question 9 : proc FORMAT permet de créer des formats d'affichage particuliers */
proc format ;
	value tranche 
	  low-<30= "faible"
	  30<-high="fort";
...
proc freq data=data.patient_t ;
	tables sexe / norow nocol ;
run;
/* Pour le test du Khi 2, ajouter l'option chisq */
proc freq data=data.patient_t ;
	tables statut*fumeur / chisq;
run;
...
proc export data=stat3 outfile='U:\TP SAS\TP2\Statistiques sur les doses des biomarqueurs 1 et 2.csv' DBMS=CSV replace;
	delimiter=";"; 
run;
...
proc ttest data=data.patient_t ;
	var dose_bmq1 dose_bmq2;
	class statut;
	ods output Ttests=result_Ttests; /* permet d'enregistrer la table Ttests sous le nom result_Ttests qui contient le résultat de la comparaison de moyenne */
run;
...
/* Affichage de la table result_Ttests */
proc print data=result_Ttests;run;


/* Question 14 */
data data.poids;
	infile 'U:\TP SAS\TP2\poids.csv' firstobs=2  DLM=';';
	input annee poids_N poids_1an;
run;

proc reg data=data.poids outest=data.stat edf tableout; /* Sauvegarde des coefficients du modèle et leur intervalle de confiance dans la table data.stat*/
	model poids_1an = poids_N;
	plot poids_1an * poids_N ; /* graphique du poids_1an en fonction du poids_N */
	plot r. * p. ; /* graphique des résidus en fonction de la prédiction du poids_1an */
	plot rstudent. * p.; /* graphique des résidus studentisés en fonction de la prédiction du poids_1an */
	output out=data.results p=prediction lcl=borne_inf ucl=borne_sup cookd=Cook; 
		/* Sauvegarde des résultats dans une table avec la prédiction du modèle, les intervalles de prédiction
	       et la distance de Cook */
run;


/* Question 15 */
data data.loyer;
	infile 'U:\TP SAS\TP2\loyer.csv' firstobs=2  DLM=';';
...
proc anova data=data.loyer outstat=stat;
	...
	means annee*situation ; /* calcul des moyennes du loyer par année et situation */
run;
...
/* Pour comparer avant/après modification des options : */
